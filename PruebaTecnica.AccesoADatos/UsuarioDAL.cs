...
namespace PruebaTecnica.AccesoADatos
{
    public class UsuarioDAL
    {
        ...
        {
            bool result = false;
            var loginUsuarioExiste = await pDbContext.Usuario.FirstOrDefaultAsync(s => s.CorreoElectronico == pUsuario.CorreoElectronico && s.Id != pUsuario.Id);
            if (loginUsuarioExiste != null && loginUsuarioExiste.Id > 0 && loginUsuarioExiste.CorreoElectronico == pUsuario.CorreoElectronico)
                result = true;
            return result;
        }
        ...
        internal static IQueryable<Usuario> QuerySelect(IQueryable<Usuario> pQuery, Usuario pUsuario)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pUsuario.CorreoElectronico))
                pQuery = pQuery.Where(s => s.CorreoElectronico.Contains(pUsuario.CorreoElectronico));
        ...
        public static async Task<Usuario> LoginAsync(Usuario pUsuario)
        {
            ...
            {
                EncriptarMD5(pUsuario);
                usuario = await bdContexto.Usuario.FirstOrDefaultAsync(s => s.CorreoElectronico == pUsuario.CorreoElectronico &&
